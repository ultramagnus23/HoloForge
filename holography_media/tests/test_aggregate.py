...
test that runs a HANDFUL of real (tiny CPU-scale) M1-shaped jobs -- not
...
(headroom_closure, gain_curve) to verify it produces sane output on
...
def _make_job(experiment_id, K, budget, method_id, seed, n_x=48, n_iters=4):
    dx = 51.2 / n_x
    ...
    config = dict(n_x=n_x, dx=dx, lam_um=0.405, n_iters=n_iters,
                 ...
                 K_nominal=K)
    return _job(experiment_id, method_id, seed, config)
    ...
    try:
        ...
        for K in Ks:
            for method_id in ["BSGD", "MIL"]:
                for seed in [0, 1]:
                    job = _make_job("M1", K, budget, method_id, seed)
        ...
        print("end-to-end headroom closure on real tiny M1-shaped data OK")
    finally:
...
def test_m3_cliff_shift_on_real_tiny_M1_and_M2_data():
    ...
    range with a genuine crossing, so both point-estimate and per-seed
    shift statistics are exercised for real."""
    tmp = tempfile.mkdtemp(prefix="agg_m3_test_")
    try:
        rm.set_results_root(tmp)
        device = rm.get_device()
        commit = rm.git_commit_hash()
        ...
        for exp_id, n_iters in [("M1", 4), ("M2", 4)]:
            for K in Ks:
                for method_id in ["BSGD", "MIL"]:
                    for seed in [0, 1]:
                        job = _make_job(exp_id, K, budget, method_id, seed, n_iters=n_iters)
                        result = rm.run_job(job, device, commit)
                        path = rm.result_path(job["experiment_id"], job["method_id"],
                                              job["config_hash"], job["seed"])
                        rm.atomic_write_json(path, result)

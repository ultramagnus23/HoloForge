...
%         from code/results/ into ./figures/ for submission, or
%         build in place against ../code/results/.
%
% NOTE ON NUMBERS: every quantitative value in this draft is taken verbatim from
% the experiment outputs in ../code/results/ produced by
...
suspect the bandwidth mask is inert there. It is not (Fig.~\ref{fig:viewing_energy}): measuring the fraction of
...
\begin{figure}[t]
  ...
  \caption{Viewing-angle operator verification. Retained hologram spectral
  energy (blue, log axis; thin grey lines per scene, thick line the mean) falls
  monotonically and by roughly two orders of magnitude as bandwidth is reduced,
  confirming the mask is active at every step. Mean reconstruction SSIM (green,
  right axis) nonetheless plateaus from $75\%$ to $25\%$ and drops sharply only at
  $10\%$, because phase-only reconstruction depends on the energy-bearing
  low-frequency core, which survives until the mask reaches $\sim\!10\%$.}
